...
\begin{definition}
	Un lenguaje $B$ es $EXPSPACE-completo$ si \cite[p. 344]{sipser2006}  
	\begin{enumerate}
	\item $B \in EXPSPACE$
	\item Todo lenguaje $A \in EXPSPACE$ es polinomialmente reducible a $B$	
	\end{enumerate}
\end{definition}
De manera similiar, para la clase de tiempo exponencial $EXPTIME$, definimos
\begin{definition}
	Un lenguaje B es $EXPTIME-completo$ si \cite[p. 2]{cita4}
	\begin{enumerate}
	\item $B \in EXPTIME$
	\item Todo lenguaje $A \in EXPTIME$ es polinomialmente reducible a $B$	
	\end{enumerate}
\end{definition}
...
\begin{proof}
	...
	\textit{Conclusión:} Como $R_1$ y $R_2$ se construyen en tiempo polinómico y capturan la aceptación de $M$, $EQ_{REX\uparrow}$ es EXPSPACE-duro.
\end{proof}
