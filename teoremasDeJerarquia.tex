...
\begin{definition} \label{def:espacio-construible}
	Una función $f : \mathbb{N} \to \mathbb{N}$, donde $f(n)$ es al menos $O(\log n)$, es llamada \textbf{espacio constructible} si la función que mapea la cadena $1^n$ a la representación binaria de $f(n)$ puede computarse utilizando espacio $O(f(n))$.\cite[p. 336]{sipser2006}
\end{definition}

\begin{definition} \label{def:tiempo-construible}
	Una función $f : \mathbb{N} \to \mathbb{N}$, donde $t(n)$ es al menos $O(\log n)$, es llamada \textbf{tiempo constructible} si la función que mapea la cadena $1^n$ a la representación binaria de $t(n)$ puede computarse en tiempo $O(t(n))$.\cite[p. 340]{sipser2006}
\end{definition}

\begin{definition}\label{def:expspace-completo}
	Un lenguaje $B$ es \textbf{EXPSPACE-completo} si:
	\begin{enumerate}
		\item $B \in \text{EXPSPACE}$, y
		\item todo lenguaje $A \in \text{EXPSPACE}$ es reducible en tiempo polinómico a $B$.
	\end{enumerate} \cite[p. 344]{sipser2006}
\end{definition}

\begin{definition}\label{def:exptime-completo}
Un lenguaje $B$ es \textbf{EXPTIME-completo} si:
\begin{enumerate}
    \item $B \in \text{EXPTIME}$, y
    \item todo lenguaje $A \in \text{EXPTIME}$ se reduce en tiempo polinómico a $B$.
\end{enumerate} \cite[p. 2]{cita4}
\end{definition}

\section{Teorema del Espacio}
Como se establece en la Definición~\ref{def:espacio-construible}, $f$ es un espacio constructible si alguna máquina de Turing M de tiempo $O(f(n))$ existe y siempre se detiene con la representacion binaria de $f(n)$ en su cinta cuando empieza en la entrada $1^n$. \cite[p. 336]{sipser2006}.
...
\begin{theorem}
  ...
  \label{thm:Teorema de la jerarquía del espacio} \cite[p. 337]{sipser2006}.
\end{theorem}
...
Esto dado a
\begin{corollary}
  Para cualesquiera 2 funciones $f_1, f_2: N \rightarrow N$, donde $f_1(n)$ es $o(f_2(n))$ y $f_2$ es espacio constructible, $SPACE(f_1(n)) \subsetneq SPACE(f_2(n))$. \cite[p. 339]{sipser2006}
\end{corollary}
...
\begin{definition}
Sea $t:\mathbb{N} \rightarrow \mathbb{N}$ una función construible en tiempo. Entonces existe un lenguaje $A$ tal que es decidible en tiempo $O(t(n))$ pero no es decidible en tiempo $o\!\left(\frac{t(n)}{\log t(n)}\right)$. \cite[p. 341]{sipser2006}
\end{definition}
