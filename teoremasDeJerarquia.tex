...
pero no es decidible en tiempo $o\!\left(\frac{t(n)}{\log t(n)}\right)$. \cite{cita1}
\end{definition}
...
Para demostrar el teorema, se define una MT $D$ diseñada para “diferenciarse” de todas las máquinas que corren en tiempo más pequeño que $t(n)/\log t(n)$.\cite{cita1} La idea es la siguiente:
...
Esta técnica es un ejemplo clásico de \textit{diagonalización}. La idea de diagonalización utilizada para construir la máquina $D$ también se 
presenta de manera intuitiva en \cite{cita2}, lo que ayuda a visualizar cómo 
$D$ garantiza diferenciarse de cualquier máquina que opere por debajo de 
$t(n)/\log t(n)$.
...
\begin{corollary}[Corolario 9.11]
...
\begin{corollary}[Corolario 9.13]
\[
...
\section{Teorema de Completitud de $EQ_{REX\uparrow}$}
\begin{definition}
	Un lenguaje $B$ es $EXPSPACE-completo$ si
	\begin{enumerate}
	\item $B \in EXPSPACE$
	\item Todo lenguaje $A \in EXPSPACE$ es polinomialmente reducible a $B$\cite{cita1}  
	\end{enumerate}
\end{definition}
De manera similiar, para la clase de tiempo exponencial $EXPTIME$, definimos
\begin{definition}
	Un lenguaje B es $EXPTIME-completo$ si
	\begin{enumerate}
	\item $B \in EXPTIME$
	\item Todo lenguaje $A \in EXPTIME$ es polinomialmente reducible a $B$\cite{cita4}
	\end{enumerate}
\end{definition}
...
\begin{theorem}
	Sea $EQ_{REX\uparrow} = \{ \langle R_1, R_2 \rangle \mid R_1 \text{ y } R_2 \text{ son expresiones regulares con exponenciación y } L(R_1) = L(R_2) \}$. $EQ_{REX\uparrow}$ es $EXPSPACE-completo$\cite{cita3}\cite{cita1}
\end{theorem}
\begin{proof}
	\begin{enumerate}
	...
	\begin{itemize}
		...
		\item $R_2$ generará todas las cadenas que \textbf{NO} son historias de computación de rechazo válidas de $M$ sobre $w$.
	\end{itemize}
